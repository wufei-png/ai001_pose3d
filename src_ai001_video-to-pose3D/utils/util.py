...
def draw_bodypose(canvas, candidate, subset, scale=1):
    ...
    for i in range(18):
        ...
        for n in range(len(subset)):
            ...
            print('x:',int(x/scale),'y:',int(y/scale))
            # keypoints[i][0]=int(x/scale)
            # keypoints[i][1]=int(y/scale)
            # print('i',i)
            j=[]
            keypoints.append([int(x/scale),int(y/scale)])
            # cv2.circle(canvas, (int(x/scale), int(y/scale)), int(dotwidth), colors[i], thickness=-1)
    print('len key',len(keypoints))
    tmp=keypoints.copy()
    tmp[0]=keypoints[0]
    tmp[1]=keypoints[14]
    tmp[2]=keypoints[13]
