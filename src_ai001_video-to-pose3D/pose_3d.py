...
from joints_detectors.Alphapose.SPPE.src.utils.eval import getPrediction
...
# plt.ion()
print('这句也执行了2次？这句位于pose_3d.py没有缩进也就是立即执行的位置')
def pose_3d():
    ...
    args.save_img = False
    args.sp = True
    # print('args.model_2d',args.model_2d)
    args.model_2d = 'alphapose'
        # 2.循环读取图片

    video_file='./outputs/Sitting1.mp4'
    ...
    for i in im_names_desc:
        ...
        with torch.no_grad():
            # global keypoint
            if(args.model_2d=='alphapose'):
                (inps, orig_img, im_name, boxes, scores, pt1, pt2)=det_processor.read()
                # print('shape(orig_img)',orig_img.shape)
                ...
                # print('result_list[..., :2]', result_list[..., :2].shape)
                keypoint = normalize_screen_coordinates(result_list[..., :2], w=1000, h=1002)
                # print('type(keypoint)',type(keypoint))
                    # copy_key=keypoint.copy()
                    # keypoint = image_coordinates(keypoint[..., :2], w=1000, h=1002)
                    # keypoint = normalize_screen_coordinates(result_list[..., :2], w=1000, h=1002)
                    # print('copy_key==keypoint',copy_key==keypoint)
                    ################ fps 3.3
                    # print('keypoint.shape',keypoint.shape) #1 17 2
            else:
                _, orig_img = cap.read()
                key = cv2.waitKey(25)
                if key == 27:  # 按键esc
                ...
                model.eval()
                print('openpose {} model is successfully loaded...'.format('MobileNet'))
                imageToTest = cv2.resize(orig_img, (0, 0), fx=float(0.3), fy=float(0.3), interpolation=cv2.INTER_CUBIC)
                # print(frame.shape)
                ...
                keypoint = draw_bodypose(orig_img, candidate, subset, float(0.3))
                # print('keypoint shaoe',keypoint.shape)
                # print('keypoint',keypoint)
                keypoint = np.array(keypoint)
                keypoint=keypoint[None,:]
                # print('keypoint',keypoint)
                keypoint = normalize_screen_coordinates(keypoint[..., :2], w=1000, h=1002)
                # print('keypoint',keypoint)
            # print('keypoint',keypoint)
            ...
            for i in range(17):
                ...
                # 左右膝 左右脚踝
                # print('x',int(input_keypoints[i][0]),'y',int(input_keypoints[i][1]))
...
def check_angle(left_leg,right_leg,back,leg_flag):
    tips=''
    say_it=False
    left_angle=cal_angle(left_leg[0],left_leg[1],left_leg[2])#这个角度计算稍微有点大
    right_angle=cal_angle(right_leg[0],right_leg[1],right_leg[2])
    ...
    if(left_angle>140 and (leg_flag['left']!=1)):
        leg_flag['left']=1
        say_it=True
        tips=tips+'左腿角度大于140度，处于放松状态，记得多活动'

    if(right_angle>140 and (leg_flag['right']!=1)):
        leg_flag['right']=1
        say_it=True
        tips=tips+'右腿角度大于140度，处于放松状态，记得多活动'
