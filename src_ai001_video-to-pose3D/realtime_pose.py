...
def get_detector_2d_rt(detector_name):
    def get_alpha_pose():
...
def main(args):
    ...
    for i in tqdm(range(get_frames()))
    # 2D kpts loads or generate
    if not args.input_npz:
        video_name = args.viz_video
        keypoints = detector_2d(video_name)#这一步很耗时间噢
    ...
    print('-------------- time124 spends {:.2f} seconds'.format(ckpt))

    keypoints_symmetry = metadata['keypoints_symmetry']
    kps_left, kps_right = list(keypoints_symmetry[0]), list(keypoints_symmetry[1])
    joints_left, joints_right = list([4, 5, 6, 11, 12, 13]), list([1, 2, 3, 14, 15, 16])
    ckpt, time123 = ckpt_time(time0)
    print('-------------- time123 spends {:.2f} seconds'.format(ckpt))
    # normlization keypoints  Suppose using the camera parameter
    keypoints = normalize_screen_coordinates(keypoints[..., :2], w=1000, h=1002)

    ckpt, time12 = ckpt_time(time0)
    print('-------------- time12 spends {:.2f} seconds'.format(ckpt))
    ...
    from common.visualization import render_animation_test#比较耗时
    from common.visualization import render_animation#比较耗时
    input_keypoints=input_keypoints[:1]
    render_animation_test(keypoints=input_keypoints,skeleton=Skeleton(),input_video_frame=)
