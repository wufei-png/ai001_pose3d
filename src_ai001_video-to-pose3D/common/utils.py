...
def evaluate(test_generator, model_pos, action=None, return_predictions=False):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            # print('batch_2d',batch_2d)
            inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
            # print('inputs_2d',inputs_2d)
            if torch.cuda.is_available():
                inputs_2d = inputs_2d.cuda()
            # Positional model
            predicted_3d_pos = model_pos(inputs_2d)
            # print('predicted_3d_pos',predicted_3d_pos)
            # print('test_generator.augment_enabled()',test_generator.augment_enabled())
            ...
            if return_predictions:
                # print('predicted_3d_pos',predicted_3d_pos.squeeze(0).cpu().numpy())
