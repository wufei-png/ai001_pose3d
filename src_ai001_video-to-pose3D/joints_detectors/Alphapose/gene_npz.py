...
args = opt
# arg.indir='.\outputs\alpha_pose_Sitting\vis'
args.dataset = 'coco'
...
def get_video_handle(video_file):
    data_loader = VideoLoader(video_file).start()
    (fourcc, fps, frameSize) = data_loader.videoinfo()
    print('the video is {} f/s'.format(fps))
    det_loader = DetectionLoader(data_loader).start()
    #  start a thread to read frames from the file video stream
    det_processor = DetectionProcessor(det_loader).start()
    print('一共',data_loader.length(),'帧')
    return det_processor,tqdm(range(data_loader.length()))
    # Load pose model
    ...
    for i in im_names_desc:
        ...
        with torch.no_grad():
            ...
            if(flag==1):
                ...
                flag=0
            # print('(inps, orig_img, im_name, boxes, scores, pt1, pt2)',(inps, orig_img, im_name, boxes, scores, pt1, pt2))
            if orig_img is None:
